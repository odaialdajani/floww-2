...
def train_rf_5y_model(ticker: str, period: str = "5y") -> dict:
    ...
    for fold in range(n_folds):
        ...
        fold_metrics.append({
            ...
            "accuracy": float(fold_acc), "train_acc": float(train_acc),
        })
        all_preds.extend(preds.tolist())
        all_actuals.extend(y_test.tolist())
        log.info(f"[{ticker}] Fold {fold}: acc={fold_acc:.4f} train_acc={train_acc:.4f}")
    ...
    manifest = {
        "ticker": ticker, "model_type": "rf", "model_id": f"{ticker}_rf_5y_v1",
        "feature_version": "v2.0", "target": "target_directional_move",
        "n_samples": n, "n_features": len(feature_cols), "feature_names": feature_cols,
        "model_params": {"n_estimators": 300, "max_depth": 5, "min_samples_leaf": 20,
                         "max_features": "sqrt", "random_state": 42},
        "metrics": {
            "overall_accuracy": float(overall_acc),
            "avg_fold_accuracy": float(wf_mean), "std_fold_accuracy": float(wf_std),
            "avg_fold_train_accuracy": float(np.mean(fold_train_accs)),
            "overfit_gap": float(overfit_gap), "overall_sharpe": float(sharpe),
            "train_time_sec": float(train_time),
        },
        "fold_metrics": fold_metrics, "top_features": top_features,
        "gate_results": gates, "verdict": verdict,
    ...
    log.info(f"[{ticker}] Saved: {model_path.name}")
...
def main():
    ...
    for ticker, result in results.items():
        if "error" in result:
            ...
            log.info(f"  {ticker}: {result.get('verdict','?')} - "
                     f"wf={m.get('avg_fold_accuracy',0):.3f}+/-{m.get('std_fold_accuracy',0):.3f} "
                     f"sharpe={m.get('overall_sharpe',0):.3f} overfit={m.get('overfit_gap',0):.3f} "

"""
Institutional flow-alert engine â€” server-side port of the scanner's alert
...
an institutional feed needs and the browser never had:
  ...
  â€¢ Black-Scholes per-contract ENTRY price (not the 0.4-approx premium)
  â€¢ side/bias inference (BUY + BULLISH/BEARISH from opening-flow shape)
  â€¢ GOLD / SILVER / BRONZE conviction tiers from a deterministic factor count
  â€¢ DuckDB-persisted feed with rule-namespaced dedup TTLs
  â€¢ move-since-alert tracking (the feed's "+3.9%" column)

Runs on EVERY fresh /scan result inside the backend, so alerts fire and
persist regardless of whether the Scanner tab is open â€” the root cause of
...
conn) â€” see floww-audit-2026-07-11 (thread-unsafe shared connection).
"""
...
# Rule-namespaced dedup TTLs (seconds) â€” mirrors the frontend's ttl choices:
...
def norm_rows(raw_rows, columns: list[str] | None = None) -> list[dict]:
    """cvforge screen list-rows â†’ normalized dicts with derived metrics.

    Malformed rows are dropped, never raised â€” a feed hiccup must not take
...
# â”€â”€ scoring (parity with scanLogic.scanScoreOf) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def scan_score(r: dict, regime: str | None = None) -> int:
    ...
    if regime == "negative" and dte is not None and dte <= 7:
        ...
        s += 3
    # Informed-positioning band (internal desk heuristic: 7â€“90 DTE +
    # volâ‰¥3أ—OI + â‰¥$25k premium is where directional bets live; shorter is
    # gamma noise, longer is hedges). Pan & Poteshman (RFS 2006) is cited
    # ONLY for the directional put-call-ratio finding â€” that paper has no
...
# â”€â”€ entry price (true Black-Scholes, not the 0.4-approx) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ side / bias inference â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def infer_side_bias(r: dict) -> tuple[str, str | None]:
    """Opening-dominant flow (vol well above resting OI) reads as initiated
    BUYing on a print-less feed; anything else is unlabeled FLOW â€” a desk
    never claims a side it can't defend.

    Paid-feed upgrade: initiation is KNOWN, not proxied, in two tiers â€”
...
# â”€â”€ tiering â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def _norm_gex_regime(raw: object) -> str | None:
    ...
    only understands the short form â€” normalize once, at the boundary.
    """
    s = str(raw or "").lower()
    if "negative" in s:
        return "negative"
...
# â”€â”€ Blademap-style weighted conviction (v3) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
# tier_of() counts booleans â€” a row with whale+cluster+CW reads GOLD the
...
_W_FLOW = 45        # vol/OI, size, notional â€” the tape itself
...
# â”€â”€ Exposure-alert â†’ conviction wiring (A3) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#
...
# every existing caller byte-identical; the آ±5 clamp keeps a structural
# overlay from ever overriding the tape read.

_EXPOSURE_KIND_WEIGHTS: dict[str, int] = {
    "vex_wall_formed": -2,    # vol suppression defended â€” fade directional prints
    "vex_wall_broken": 3,     # suppression released â€” regime may shift, tradable
    "charm_pin_formed": 2,    # hedging concentration into expiry â€” follow-through
    "charm_pin_shifted": 1,   # magnet moved strikes â€” weak continuation
}
...
def exposure_adjustment_for_events(events: list[dict] | None) -> int:
    ...
    total to [-5, +5]. Empty/None input â†’ 0.
    """
    total = 0
...
def score_conviction(r: dict, factors: dict | None = None,
    ...
    GEX confluency weighted heaviest (paper-accurate خ“IB is our hardest
    ...
    # as before â€” this rewards MEASURED urgency, never a proxy.
...
# â”€â”€ Blademap alert contract: key levels + context â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# the GEX regime (short gamma moves harder â€” Ni-Pearson 2020).
...
# â”€â”€ the engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def minutes_since_open_now() -> float | None:
    """Minutes since today's 09:30 ET open (seconds-precision frozen at call
    time). None outside RTH â€” the caller freezes None honestly rather than a
...
def _mk_alert(r: dict, rule: str, extra: dict, factors: dict, asof: str) -> dict:
    ...
    a = {
        ...
        # laddered with â‰¥3 same-bias qualifying contracts in the snapshot
        ...
        # feed had no two-sided quote). Kyle-خ» arrives separately (B).
...
def _finalize(a: dict, r: dict, cw_map: dict | None) -> dict:
    ...
    paired strategy legs â€” a vertical's leg is never a directional whale."""
    cw = (cw_map or {}).get(a["under"])
    a["cw_spread"] = round(cw, 4) if cw is not None else None
    if r.get("spread_leg"):
        a["side"], a["bias"], a["tier"] = "STRATEGY", None, "BRONZE"
        a["why"] = (a.get("why") or "") + " [paired legs: likely spread â€” direction unclaimed]"
    return a
...
def _common_factors(r: dict, regimes: dict, sigma_tickers: set,
    ...
    # it). A flat {"gamma_imbalance": ...} payload is ALSO accepted â€” the
    ...
    if gex_context:
        ...
        # propagate the regime, never confluence â€” None must not compare.
        ...
        # flow. infer_side_bias returns "BULLISH"/"BEARISH" uppercase â€” compare
...
def eval_institutional(rows, baselines=None, prev_oi=None, regimes=None, opts=None,
    ...
    PRIME > 0DTE), plus per-ticker SIGMA and CLUSTER alerts. Pure logic â€”
    ...
    sub-whale mid-cap bracket that never clears SCORE 92 â€” the SNDK gap.
    """
    ...
    o = {
        # 2026-09-02 institutional noise pass â€” mirrors the frontend's tightened
        # defaults (scanLogic.js / FlowseekerProBlademap.jsx DEFAULT_RULES):
        # SCORE 85â†’92, WHALE $10Mâ†’$25M, SIGMA 3.0â†’6.0. Parity contract: the
        ...
        **DEFAULT_EVAL_OPTS,
        ...
        # every fired alert gains p_move/p_method/p_n â€” server-computed,
    ...
    # spread, and same-bias laddering â€” all cross-row reads of THIS snapshot.
    detect_spreads(rows)
    cw_map = cw_iv_spread(rows)
    clusters = cluster_biases(rows)

    # Per-ticker دƒ vs multi-day baseline. Two-stage quality gate:
    #   1. market-mode removal â€” a broad volume day lifts every ticker's دƒ
    #      together; subtract the cross-sectional median (when the cross-
    #      section is wide enough to define one) so only IDIOSYNCRATIC
    #      spikes remain;
    #   2. Benjamini-Hochberg FDR at q â€” testing hundreds of tickers a day
    ...
    # Pass 1 â€” OICONF: overnight OI build is the one hard "yesterday's flow
    # was real" proof a print-less feed offers. Top 5 by % build.
    # خ”OI hygiene (services/oi_hygiene.py, 2026-09-02): rollover/expiring
    # contracts are skipped entirely â€” a roll's next-expiry pop is migration,
    ...
    for pct, add_notl, r, tag in cand[:5]:
        ...
        a = _finalize(_mk_alert(r, "OICONF", {
            ...
            "why": f"OI +{round(pct * 100)}% overnight (${add_notl / 1e6:.1f}M added notional) â€” prior-day flow HELD as new positioning",
        }, f, asof), r, cw_map)
    ...
    # Pass 2 â€” intraday per-contract rules, strongest first, one per contract.
    for r in rows:
        ...
        if score >= o["min_score"]:
            ...
            rule = "0DTE"
            why = f"{r['dte']} DTE with score {score} â€” urgent short-fuse positioning"
        if not rule:
            continue
        out.append(_finalize(_mk_alert(r, rule, {"why": why}, f, asof), r, cw_map))

    # Pass 3 â€” per-ticker SIGMA alerts (aggregate anomaly, no single strike).
    for under, s in sigma_hits.items():
        ...
        a = _mk_alert(best, "SIGMA", {
            "sigma": s,
            "why": f"{under} options volume {s}دƒ above its {b.get('days')}-day baseline",
        }, f, asof)
        a["key"] = f"sigma|{under}"
        a["ckey"] = under
    ...
    # Pass 4 â€” per-ticker CLUSTER alerts (laddered accumulation in ONE
    # snapshot: >=3 same-bias qualifying contracts). The SNDK read â€” steady
    ...
    for under, bias in (clusters or {}).items():
        ...
        a = _mk_alert(best, "CLUSTER", {
            "why": (f"{under} laddered {bias} accumulation â€” {len(legs)} opening-shaped "
    ...
    # Calibration provenance â€” attach the server-computed p_move to every
    # fired alert. Stage-0 model â†’ p_move=None + "uncalibrated" on each row:
    ...
    # change gated on stage â‰¥ 1 by design â€” never let None block a fire.
...
# â”€â”€ DuckDB I/O â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def init_flow_alert_tables(engine) -> None:
    ...
    with contextlib.suppress(Exception):
        # Migration for v2.1 â€” cluster factor surfaced to the feed (2026-07-20).
        engine.execute_write(
            "ALTER TABLE flow_alerts_daily ADD COLUMN IF NOT EXISTS cluster BOOLEAN")
    with contextlib.suppress(Exception):
        # Migration for v2.3 â€” wins column (BIGINT). The alert_quality() SQL
...
def conviction_calibration(engine, days: int = 30) -> list[dict]:
    """Blademap v3 â€” hit-rate by CONVICTION BAND (the score's own report card).

    Buckets: 50-59 / 60-74 / 75+ (the sizing tiers in flow_trade_bridge).
    hit = same â‰¥0.5%-in-direction threshold as alert_quality. If the curve
    isn't monotonic (75+ hitting no better than 50-59), conviction sizing
    is decoration and the weights need retuning â€” this table is how the
    desk sees that.

    Bands with zero measured alerts still appear (n_measured=0) so the
    frontend renders an honest "â€”" rather than dropping the row.
    """
...
def alert_quality(engine, days: int = 30) -> list[dict]:
    """Per rule أ— tier precision from realized moves â€” the calibration loop.

    hit = the underlying moved â‰¥0.5% in the alert's claimed direction since
    the alert fired (move_pct is stamped by update_moves on every scan).
    Directionless rows (bias NULL â€” STRATEGY legs, SIGMA) are excluded.
    ...
    v2.x contract â€” also returns `sigma_median` per (rule, tier) row (DuckDB
    ...
    DESC; min-n floor = 3; tier must have â‰¥2 qualifying rules. The boolean is
    ...
    # Per-row is_best_rule flag â€” Python-side ranking mirrors the frontend
    ...
    _BEST_RULE_MIN_N = 3  # mirrors convictionUi.js BEST_RULE_MIN_N â€” keep identical
    for r in rows:
...
def alert_quality_daily(engine, tier: str | None = None, days: int = 30) -> list[dict]:
    """v2.5 â€” daily (date, tier) precision series for the per-tier sparkline.
    ...
    (bias NULL) are excluded â€” they cannot claim a directional hit by
    definition.
    ...
    gaps â€” the gap is information (no measured alerts = no signal), the
...
def dedup_filter(engine, alerts, now: float | None = None) -> list[dict]:
    ...
    try:
        ...
        # but it MUST be observable â€” silent dedup loss = duplicate alert spam.
        logger.warning(f"flow_alerts.dedup query failed ({e}) â€” dedup disabled this cycle")
        seen = {}
    kept = []
    for a in alerts:
        s = seen.get(a["key"])
...
def update_moves(engine, spot_map: dict, stamp_ts: str | None = None) -> int:
    """Stamp the latest underlying price onto open alerts â†’ move-since-alert.
    ...
    flow_alert_moves (fail-open â€” a leg-write failure never blocks the
...
def horizon_moves(engine, asof_date: str, key: str,
    ...
    calendar needed at write time). Unmeasured horizons are None â€” honest
...
def read_alert_feed(engine, days: int = 7, min_tier: str | None = None,
    ...
    sort_by="conviction" (Blademap): ranked conviction DESC â€” a 92-conviction

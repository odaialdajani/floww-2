...
class FinnhubClient:
    ...
    def all_symbols(self) -> list[dict[str, Any]] | None:
        """List of all US symbols via Finnhub.

        Uses the real ``stock_symbols('US')`` SDK call (there is no
        ``symbol_list`` method — a previous version called it and the
        universe silently stayed empty). Items carry at least ``symbol``.
        """
        if not self._client:
            return None
        try:
            return self._client.stock_symbols("US")
        except Exception as e:

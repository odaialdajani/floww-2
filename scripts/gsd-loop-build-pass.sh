...
if [ -z "$EVENT" ]; then
  ...
  if [ "$REASON" = "tcc-documents" ]; then
    log "REMEDIATION: launchd cannot read $REPO (macOS TCC: ~/Documents). Grant Full Disk Access to /bin/zsh (System Settings > Privacy & Security), then set paused=false in $STATE_FILE. Until then, run passes manually from Terminal: ./scripts/gsd-loop-build-pass.sh"
  fi
fi
